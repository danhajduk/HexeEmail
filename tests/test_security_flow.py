...
def test_security_phase4_builds_ai_template_hook_for_unresolved_result(tmp_path):
    result = asyncio.run(
        SecurityFlowPipeline(runtime_dir=tmp_path / "runtime").process_normalized_email(
            build_security_phase1_payload()
        )
    ...
    assert phase4.ai_template_hook["profile_id"] == "security_alert"
    assert phase4.ai_template_hook["profile_family"] == "security"
    assert phase4.ai_template_hook["scrubbed_text"]


def test_security_pipeline_creates_probation_template_for_unresolved_profile(tmp_path):
    ...
    async def fake_generate(request):
        ...
        path = store.save_template_payload(
            request.template_id,
            {
                "schema_version": "security-phase4-template.v1",
