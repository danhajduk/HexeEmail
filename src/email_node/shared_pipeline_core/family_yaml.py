...
FlowFamilyYamlName = Literal["order", "action_required", "financial", "invoice"]
...
def parse_yaml_family_reference(reference: str) -> FlowFamilyYamlName:
    ...
    if family_name not in {"order", "action_required", "financial", "invoice"}:
...
def _canonicalize_yaml_flow_family(flow_family: FlowFamilyYamlName | str) -> FlowFamilyYamlName:
    if flow_family == "action_needed":
        return "action_required"
    if flow_family in {"order", "action_required", "financial", "invoice"}:
