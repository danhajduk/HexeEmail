...
class SharedTemplateRegistry:
    ...
    def validate_template(self, template: dict[str, object]) -> list[str]:
        ...
        if template.get("schema_version") != self.schema_version:
        ...
        for field_name, rule in extract.items():
            ...
            if method not in self.supported_extraction_methods:
...
class SharedTemplateExecutionEngine:
    def __init__(
        self,
        *,
    ...
    def extract(self, phase3: GmailPhase3DetectedEmail) -> GmailPhase4ExtractedEmail:
        ...
        if working is None:
            ...
            return GmailPhase4ExtractedEmail(
                ...
                normalization_metadata=self._normalization_metadata(),
            )
    ...
    def _normalization_metadata(self) -> GmailPhase4NormalizationMetadata:
        return GmailPhase4NormalizationMetadata(
