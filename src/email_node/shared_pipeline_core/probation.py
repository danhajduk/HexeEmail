...
class SharedProbationStore(Protocol):
    def load_template_payload(self, template_id: str) -> dict[str, object] | None: ...

    def save_evaluation(self, evaluation) -> object: ...


class SharedPromotionService(Protocol):
    def promote(self, template_id: str) -> object: ...
...
class SharedProbationEvaluator:
    def __init__(
        self,
        *,
        probation_store: SharedProbationStore,
        extractor: SharedProbationExtractor,
    ) -> None:
        self.probation_store = probation_store
        self.extractor = extractor

    def evaluate(self, phase3, *, template_id: str) -> SharedProbationEvaluationResult:
        ...
        if not isinstance(template, dict):
            result = SharedProbationEvaluationResult(
...
class SharedProbationMetrics:
    @staticmethod
    def update_state(
        state: SharedProbationTemplateState,
        result: SharedProbationEvaluationResult,
    ) -> SharedProbationTemplateState:
...
class SharedProbationPromotionPolicy:
    ...
    def is_promotion_eligible(self, state: SharedProbationTemplateState) -> bool:
        return (
    ...
    def should_reject_template(self, state: SharedProbationTemplateState) -> bool:
        if state.sample_count < self.minimum_sample_count:
            return False
        return state.hard_failure_count > self.hard_failure_count_max + 1

    def should_mark_for_refinement(self, state: SharedProbationTemplateState) -> bool:
        if state.sample_count < self.minimum_sample_count:
            return False
    ...
    def should_remain_on_probation(self, state: SharedProbationTemplateState) -> bool:
...
class SharedProbationPromotionManager:
    def __init__(
        self,
        *,
        promotion_service: SharedPromotionService,
        policy: SharedProbationPromotionPolicy | None = None,
    ) -> None:
        self.promotion_service = promotion_service
        self.policy = policy or SharedProbationPromotionPolicy()

    def evaluate_and_apply(self, state: SharedProbationTemplateState) -> SharedProbationTemplateState:
...
def build_probation_shadow_comparison(phase4, evaluation: SharedProbationEvaluationResult) -> dict[str, object]:
    active_fields = {
