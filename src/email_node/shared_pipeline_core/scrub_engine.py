...
def load_scrub_heuristic_pack(module_path: str) -> SharedScrubHeuristicPack:
    ...
    return SharedScrubHeuristicPack(
        ...
        stop_marker_patterns=list(module.STOP_MARKER_PATTERNS),
        chrome_line_patterns=list(module.CHROME_LINE_PATTERNS),
        footer_cutoff_patterns=list(module.FOOTER_CUTOFF_PATTERNS),
        important_link_patterns=dict(module.IMPORTANT_LINK_PATTERNS),
        tracking_host_patterns=list(module.TRACKING_HOST_PATTERNS),
        filler_entity_patterns=list(module.FILLER_ENTITY_PATTERNS),
        transactional_anchor_patterns=list(module.TRANSACTIONAL_ANCHOR_PATTERNS),
        promo_marker_patterns=list(module.PROMO_MARKER_PATTERNS),
    )


class SharedScrubEngine:
    def __init__(self, *, heuristic_pack: SharedScrubHeuristicPack, scrubber_version: str) -> None:
        self.heuristic_pack = heuristic_pack
        self.scrubber_version = scrubber_version
    ...
    def select_transactional_content(self, *, primary_text: str, fallback_text: str) -> tuple[str, list[str], list[str], str]:
        candidates = [
    ...
    def strip_hidden_content(self, text: str) -> tuple[str, list[str]]:
        updated = text
        applied: list[str] = []
        for pattern in self.heuristic_pack.filler_entity_patterns:
    ...
    def remove_email_chrome(self, lines: list[str]) -> tuple[list[str], list[str], bool]:
        ...
        for line in lines:
            stripped = line.strip()
            if stripped and any(pattern.search(stripped) for pattern in self.heuristic_pack.chrome_line_patterns):
    ...
    def apply_footer_cutoff(self, lines: list[str]) -> tuple[list[str], str | None]:
        for index, line in enumerate(lines):
            ...
            if not stripped:
                continue
            if any(pattern.search(stripped) for pattern in self.heuristic_pack.stop_marker_patterns + self.heuristic_pack.footer_cutoff_patterns + self.heuristic_pack.promo_marker_patterns):
    ...
    def apply_seller_agnostic_rules(self, lines: list[str]) -> tuple[list[str], list[str], int]:
        ...
        for line in lines:
            ...
            if not stripped:
                kept.append("")
                continue
            if any(pattern.search(stripped) for pattern in self.heuristic_pack.ignore_line_patterns):
    ...
    def suppress_recommendation_lines(self, lines: list[str]) -> tuple[list[str], list[str]]:
        ...
        for line in lines:
            ...
            if not stripped:
                kept.append(line)
                continue
            if any(pattern.search(stripped) for pattern in self.heuristic_pack.promo_marker_patterns):
    ...
    def normalize_semantic_lines(self, lines: list[str]) -> list[str]:
        ...
        for raw_line in lines:
            ...
            if self._low_value_line(line):
                continue
            if any(pattern.search(line) for pattern in self.heuristic_pack.promo_marker_patterns):
                continue
    ...
    def _classify_link(self, *, label: str | None, url: str) -> str:
        text = f"{label or ''} {url}".strip()
        for link_type, pattern in self.heuristic_pack.important_link_patterns.items():
    ...
    def _build_link(self, *, url: str, label: str | None, source: str, raw_url: str | None = None) -> GmailPhase2Link:
        ...
        return GmailPhase2Link(
            ...
            is_tracking=any(pattern.search(normalized_url or url) for pattern in self.heuristic_pack.tracking_host_patterns),
            is_valid=is_valid,
    ...
    def _score_transactional_block(self, block: str) -> tuple[int, list[str]]:
        ...
        reasons: list[str] = []
        lowered = block.lower()
        for pattern in self.heuristic_pack.transactional_anchor_patterns:
        ...
        for pattern in self.heuristic_pack.promo_marker_patterns:
            if pattern.search(lowered):
                score -= 3
    ...
    def _is_likely_transactional_email(self, text: str) -> bool:
        lowered = text.lower()
        return any(pattern.search(lowered) for pattern in self.heuristic_pack.transactional_anchor_patterns)
    ...
    def _is_promo_block(self, block: str) -> bool:
        lowered = block.lower()
        promo_hits = sum(1 for pattern in self.heuristic_pack.promo_marker_patterns if pattern.search(lowered))
        if promo_hits:
            return True
